...
class CnnDqnAgent(object):
    policy_frozen = False
    epsilon_delta = 1.0 / 10 ** 4.4# print '%.10f' %(1.0 / 10 ** 4.4)0.0000398107170553496878006617676337697275812388397753238677978515625
    min_eps = 0.1
    ...
    def agent_start(self, observation):
        ...
        print return_action, type(return_action)#---------------------------------------------------
    ...
    def agent_step(self, reward, observation):
        ...
        return action, eps, q_now, obs_array# server.py 120行 self.agent.agent_step_update(reward, action, eps, q_now, obs_array)

    def agent_step_update(self, reward, action, eps, q_now, obs_array):
        # Learning Phase
        if self.policy_frozen is False:  # Learning ON/OFF
            self.q_net.stock_experience(self.time, self.last_state, self.last_action, reward, self.state, False)#self.state= st+1
        ...
        print('Step:%d  Action:%d  Reward:%.1f  Epsilon:%.6f  Q_max:%3f' % (
            self.time, self.q_net.action_to_index(action), reward, eps, q_max))# ￥Step:92  Action:0  Reward:0.0  Epsilon:1.000000  Q_max:0.000000
