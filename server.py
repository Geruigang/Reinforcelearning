...
class AgentServer(WebSocket):
    ...
    def received_message(self, m):
        ...
        #print observation["image"][0], "observation" #observation["image"]=[<PIL.PngImagePlugin.PngImageFile image mode=RGB size=227x227 at 0x7F60F28>]
        ...
        if not self.agent_initialized:
            ...
            if end_episode:#如果在大如果下 结束这一回合，通过开始的state_获得动作和q值，然后下个状态就是开始的状态
                ...
                if action != 2:
                    reward -= 0.5
                self.send_action(action)
                self.agent.agent_step_update(reward, action, eps, q_now, obs_array)#通过cnn——agentupdata self.q_net.stock_experience和 self.q_net.experience_replay
